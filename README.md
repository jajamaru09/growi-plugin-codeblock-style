...
| `filename` | 文字列 | なし | `:filename=app.tsx` | コードブロック右上にファイル名をオーバーレイ表示 |
...
````
```prism-typescript:toolbar:filename=app.tsx
export const App = () => <div>Hello</div>;
```
````

`prism-` を付けない GROWI 標準の構文 `言語:ファイル名` でも、同じくファイル名がオーバーレイ表示されます。
